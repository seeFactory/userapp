...
- 点击图片作品会调用当前平台的全屏预览；微信小程序使用原生图片预览，支持多结果左右滑动和长按菜单，作品列表仍保持点击进入详情。
- 视频缺少图片封面时，H5 捕获首帧作为预览，用户点击后再挂载播放组件。
- 作品发布、下载限制和失败原因均以后端返回为准。
- 排队中或生成中的任务会自动轮询状态，并提供立即刷新和取消任务入口。
- H5 保存会打开下载地址；微信小程序仅通过 `SEEFACTORY_WEAPP_DOWNLOAD_ORIGINS` 配置的 HTTPS 白名单域名下载，默认使用 `https://sf-oss.sidcloud.cn`，旧 OSS 原始域名会回退到作品的白名单 CDN 地址。
- 小程序保存远程图片或视频时先下载为临时文件再写入相册；相册权限被拒绝后会引导打开设置并在授权后重试。
...
- 线上 9 个图文生图模式现提供可选 `0-6` 张参考图片；7 个图文生视频模式保持可选单图，1 个严格图生视频模式保持必选单图，2 个纯文生视频模式不显示参考图入口。
- 八个运行端的生成核心契约增加多图追加、剩余数量和旧作品素材迁移检查；H5 与微信小程序正式编译通过。
- 生产两个 API 域名均回读 18 个工具、19 个模式、43 个渠道和 27 个可选渠道；本轮未改变价格、渠道置灰状态，也未发起真实生图或视频请求。

### 2026-07-24 - 微信作品图片全屏预览修复

- 修复作品详情图片只展示、无法响应点击的问题；图片作品现在显式启用交互，并调用微信原生 `previewImage` 进入全屏预览。
- 同一作品包含多个结果图片时，可在原生预览中左右滑动；同时启用微信长按菜单。视频播放逻辑保持不变。
- 全屏预览仅在作品详情启用，作品广场和“我的作品”卡片继续点击进入详情，不会被图片预览拦截。
- 微信实现拆分到平台专用模块，避免把完整 Taro API 实现引入 H5 公共包；H5 体积预算、生成核心回归和微信实际编译均通过。
